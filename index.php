...
$pageTitle = 'Saffron & Spice | Premium Spice Boutique';
$heroSlides = get_hero_slides();
$bestSellers = get_best_sellers();
$collectionProducts = get_collection_products();
            ...
            >
              <div class="hero-copy">
                <?php if (!empty($slide['tag_line'])): ?>
                  <span class="tag"><?= htmlspecialchars($slide['tag_line']) ?></span>
                <?php endif; ?>
                ...
                <div class="actions">
                  <?php if (!empty($slide['primary_label']) && !empty($slide['primary_url'])): ?>
                    <a class="button primary" href="<?= htmlspecialchars($slide['primary_url']) ?>">
                      <?= htmlspecialchars($slide['primary_label']) ?>
                    </a>
                  <?php endif; ?>
                  <?php if (!empty($slide['secondary_label']) && !empty($slide['secondary_url'])): ?>
                    <a class="button outline" href="<?= htmlspecialchars($slide['secondary_url']) ?>">
                      <?= htmlspecialchars($slide['secondary_label']) ?>
                    </a>
                  <?php endif; ?>
                </div>
              </div>
              <div class="hero-media">
                <?php
                  ...
                  foreach ($orbs as $orbClass):
                    $class = 'spice-orb ' . htmlspecialchars(str_replace(' ', ' ', $orbClass));
                ...
                <?php if (!empty($slide['card_title']) || !empty($slide['card_subtitle']) || !empty($slide['price'])): ?>
                  <div class="floating-card">
                    <?php if (!empty($slide['card_title'])): ?>
                      <h3><?= htmlspecialchars($slide['card_title']) ?></h3>
                    <?php endif; ?>
                    <?php if (!empty($slide['card_subtitle'])): ?>
                      <p><?= htmlspecialchars($slide['card_subtitle']) ?></p>
                    <?php endif; ?>
                    <?php if (!is_null($slide['price'])): ?>
          ...
          <?php foreach ($bestSellers as $product): ?>
            <article class="product-card highlight">
              ...
              <div class="card-footer">
                ...
                <a class="button ghost" href="/online-sp/product.php?slug=<?= urlencode($product['slug']) ?>">
                  View details
                </a>
              </div>
            </article>
